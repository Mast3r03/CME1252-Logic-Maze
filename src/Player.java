public class Player
{
    ...
    public void performMove(int directionType)
    {
        ...
        if (directionType == GameLoop.Direction.UP)    { dy = -1; }
        if (directionType == GameLoop.Direction.DOWN)  { dy =  1; }
        if (directionType == GameLoop.Direction.LEFT)  { dx = -1; }
        if (directionType == GameLoop.Direction.RIGHT) { dx =  1; }

        this.col = this.col + dx;
        this.row = this.row + dy;
        this.moveCooldown = GameLoop.PLAYER_MOVE_INTERVAL;
    }
